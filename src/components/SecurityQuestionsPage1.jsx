// src/components/SecurityQuestionsPage1.jsx - PASO 3: PREGUNTAS DE SEGURIDAD (REGISTRO COMPLETO)
...
const SecurityQuestionsPage1 = ({ registrationData, onNext, onBack }) => {
  ...
  const loadAvailableQuestions = async () => {
    setIsLoading(true);
    try {
      console.log('❓ [QUESTIONS-REG] Cargando preguntas de seguridad disponibles');
      const result = await apiService.getAvailableSecurityQuestions();
      
      if (result.success) {
        console.log('✅ [QUESTIONS-REG] Preguntas cargadas:', result.data.questions.length);
        ...
        console.error('❌ [QUESTIONS-REG] Error cargando preguntas:', result.error);
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('📝 [QUESTIONS-REG] Validando preguntas antes de continuar');
    ...
    try {
      // Preparar preguntas válidas para enviar al siguiente paso
      const validQuestions = selectedQuestions.filter(q => q.codigo && q.respuesta.trim());
        ...
        <div className={`w-full max-w-2xl transition-all duration-1000 ${
          ...
          <div className="mb-6">
            <button
              onClick={onBack}
              disabled={isSaving}
              className="flex items-center space-x-2 text-slate-700 hover:text-slate-900 bg-white/80 hover:bg-white px-3 py-2 rounded-lg backdrop-blur-sm border border-slate-300 transition-all duration-200 disabled:opacity-50 font-semibold"
            >
          ...
          <div className="mb-8">
            ...
            <div className="text-center mt-3">
              <p className="text-slate-800 text-sm font-bold">
                Paso 3 de 4: Preguntas de Seguridad
              </p>
            </div>
          </div>

          {/* Main card - ESTILO ACTUALIZADO COMO SECURITYQUESTIONSPAGE */}
          <div className="backdrop-blur-xl bg-white/95 rounded-2xl p-5 shadow-2xl border border-white/50 relative overflow-hidden">
            ...
            {/* Alert - ESTILO ACTUALIZADO COMO SECURITYQUESTIONSPAGE */}
            {alert && (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {/* Preguntas de seguridad - ESTILO ACTUALIZADO COMO SECURITYQUESTIONSPAGE */}
              ...
              {/* Información importante - ESTILO ACTUALIZADO COMO SECURITYQUESTIONSPAGE */}
...
export default SecurityQuestionsPage1;